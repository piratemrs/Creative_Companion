...
public class RadialMenuItem extends Group implements ChangeListener<Object> {
	...
	protected ObjectProperty<Paint> backgroundMouseOnFill = new SimpleObjectProperty<Paint>(Color.LIGHTGRAY);

	protected ObjectProperty<Paint> backgroundFill = new SimpleObjectProperty<Paint>(Color.GRAY);
	...
	protected ObjectProperty<Paint> strokeFill = new SimpleObjectProperty<Paint>(Color.GRAY);

	protected ObjectProperty<Paint> strokeMouseOnFill = new SimpleObjectProperty<Paint>(Color.LIGHTGRAY);
	...
	protected void computeCoordinates() {
		...
		if (!clockwise.get()) {
			innerStartX = innerRadiusValue * Math.cos(Math.toRadians(startAngleValue));
			innerStartY = -innerRadiusValue * Math.sin(Math.toRadians(startAngleValue));
			innerEndX = innerRadiusValue * Math.cos(Math.toRadians(startAngleValue + length));
			innerEndY = -innerRadiusValue * Math.sin(Math.toRadians(startAngleValue + length));

			innerSweep = false;

			startX = radiusValue * Math.cos(Math.toRadians(startAngleValue + length));
			startY = -radiusValue * Math.sin(Math.toRadians(startAngleValue + length));
			...
			if (graphic.get() != null) {
				graphicX = graphicRadius * Math.cos(Math.toRadians(graphicAngle))
						- graphic.get().getBoundsInParent().getWidth() / 2.0;
				graphicY = -graphicRadius * Math.sin(Math.toRadians(graphicAngle))
						- graphic.get().getBoundsInParent().getHeight() / 2.0;

			}
			translateX = offsetValue * Math.cos(Math.toRadians(startAngleValue + (length / 2.0)));
			translateY = -offsetValue * Math.sin(Math.toRadians(startAngleValue + (length / 2.0)));
	...
	@Override
	public void changed(final ObservableValue<? extends Object> observableValue, final Object previousValue,
